...
BENCHMARK_TOOL_CALLS = [
    ...
    {"add_places": [{"query": "Lang rau Tra Que", "day": 2}]},
    ...
    {"activity_preferences": {"activities": ["local_experience"], "mode": "add"}},
    ...
    {"add_places": [{"query": "Lang gom Thanh Ha", "day_strategy": "most_free"}]},
    {"optimization_policy": {"reorder_only": True, "minimize_travel": True}},
...
def test_flat_trip_settings_from_model_are_normalized():
    parsed = ApplyTripChangesInput.model_validate({
        "duration": 1,
        "region": DA_NANG,
    ...
    assert parsed.trip_settings.duration == 1
    assert parsed.trip_settings.region == DA_NANG
    assert parsed.trip_settings.location_focus == "Sơn Trà"
    assert parsed.trip_settings.location_mode == "nearby"


@pytest.mark.parametrize("unsupported_field", ["meal_preferences", "meal_requests"])
def test_mvp_schema_does_not_expose_meal_planning(unsupported_field):
    with pytest.raises(ValueError):
        ApplyTripChangesInput.model_validate({
            unsupported_field: (
                {"preferences": ["local_food"]}
                if unsupported_field == "meal_preferences"
                else [{
                    "day": 1,
                    "meal_slot": "lunch",
                    "preferences": ["local_food"],
